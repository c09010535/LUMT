...
)
{
    ...
    nnz = p [n_col] ; // number of non-zeros
    if (nnz < 0)	/* nnz must be >= 0 */
    {
	stats [COLAMD_STATUS] = COLAMD_ERROR_nnz_negative ;
	stats [COLAMD_INFO1] = nnz ;
	...
	stats [COLAMD_STATUS] = COLAMD_ERROR_A_too_small ; // (-7)
...
)
{
	...
	for (row = 0; row < n_row; row++) {
		printf(" Row[%d]:", row);
		for (col = 0; col < Row[row].length; col++) {
			printf(" (%d) ", A[p[n_col] + counter + col]);
		}
		printf("\n");
		counter += Row[row].length;
	}*/
...
)
{
    ...
    {
	dense_row_count = DENSE_DEGREE (knobs [COLAMD_DENSE_ROW], n_col) ; // Defaulted value of knobs [COLAMD_DENSE_ROW] is 10
    }
    ...
    {
	dense_col_count =
	    DENSE_DEGREE (knobs [COLAMD_DENSE_COL], MIN (n_row, n_col)) ; // Defaulted value of knobs [COLAMD_DENSE_COL] is 10
    }
	...
	while (cp < cp_end)
	{
	    ...
	    *new_cp++ = row ;
	    ...
	    score += Row [row].shared1.degree - 1 ; // Remove self-self degree
	...
	if (COL_IS_ALIVE (c))
	{
		...
		//printf("col[%d] score %d next_col %d;\n", c, score, next_col);
    ...
    *p_max_deg = max_deg ;
	...
	/*for (row = 0; row < n_row; row++) {
		printf(" Row_degree[%d] %d\n", row, Row[row].shared1.degree);
	}*/
}
...
)
{
    ...
    tag_mark = clear_mark (0, max_mark, n_row, Row) ; // If the input tag_mark <= 0 or >= max_mark, do clear
    min_score = 0 ;
    ngarbage = 0 ;
    DEBUG1 (("colamd: Ordering, n_col2=%d\n", n_col2)) ;
	//printf("colamd: Ordering, n_col2 = %d\n", n_col2) ;
    /* === Order the columns ================================================ */

	/*for (k = 0; k < n_col; k++) {
		printf(" Col[%d] thick %d;\n", k, Col[k].shared1.thickness);
	}*/
	...
	//printf(" k[%d] (0) min_score = %d;\n", k, min_score);
	while (head [min_score] == EMPTY && min_score < n_col)
	{
	    min_score++ ;
	}
	//printf("       (1) min_score = %d;\n", min_score);
	pivot_col = head [min_score] ; // The pivot column has the minimum score
	ASSERT (pivot_col >= 0 && pivot_col <= n_col) ;
	//printf("       (2) pivot_col = %d;\n", pivot_col);
	...
	//printf("  pivot score = %d;\n", pivot_col_score);

	/* the pivot column is the kth column in the pivot order */
	Col [pivot_col].shared2.order = k ;

	/* increment order count by column thickness */
	pivot_col_thickness = Col [pivot_col].shared1.thickness ; // The thickness of all columns are initially set to be 1 in the function 'init_rows_cols'.
	//printf(" pivot column thickness = %d\n", pivot_col_thickness);
	...
	while (cp < cp_end)
	{
		...
		while (rp < rp_end)
		{
			...
			//printf(" k[%d] Row %d, Col %d, thick %d\n", k, row, col, col_thickness);
	...
	while (rp < rp_end)
	{
		...
		row_mark = Row [row].shared2.mark ;
		//printf(" k[%d] row_mark[%d] %d\n", k, row, row_mark); fflush(stdout);
		/* skip if dead */
		if (ROW_IS_MARKED_DEAD (row_mark))
		{
		    DEBUG4 ((" Row %d, dead\n", row)) ;
...
)
{
    ...
    pdest = &A[0] ; // Start of the row indexes in the CSC form
    for (c = 0 ; c < n_col ; c++)
    {
	if (COL_IS_ALIVE (c))
	{
    ...
    psrc = pdest ; // 'pdest' is the ending of the CSC form, and is also the start of the CSR form
    while (psrc < pfree)
    {
